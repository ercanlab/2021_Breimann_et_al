...
%%%%Written by:
%%%% Andrew Woehler, Laura Breimann, Kustrim Cerimi and Vic-Fabienne Schumann 
...
data_dir = uigetdir(pwd, 'choose data_tif directory') %choose input directory
 
disp('Choose matlab_results directory')
results_dir = uigetdir(pwd,'Choose matlab_results directory') %choose output directory
...
% compare pre and post filter, uncomment if you don't want to have the controls during image inspection 
...
disp('Step 6, exponential fitting,  Save graph as bleaching_correction.tiff to the output folder')
...
title('Fit for acquisition bleaching');
...
ylim([0 1.1]);
title('Acquisition bleaching corrected background');
box off; ylim([0 1.5]); 
...
title('Raw fluorescence in bleach-point');
box off; 
...
title('Corrected fluorescence in bleach-point');
box off; ylim([0 1.5]); 
xlabel('time [s]'); ylabel('Norm. Int.');

filename2 = strcat(strtok(fullfile(results_dir,filename_new),'.'),'_bleaching_correction') 
...
disp('Step 7 recovery fitting,  Save graph as recovery.tiff to the output folder')
...
filename3 = strcat(strtok(fullfile(results_dir,filename_new),'.'),'_recovery')  
...
filename4 = strcat(strtok(fullfile(results_dir,filename_new),'.'),'_thalf_no_fit')  
...
% if you want to reanalyse the data in the future, you can save the
% workspace that will save you the roi setting steps. Uncomment the next two lines: 
...
%export different t-half values for Python analysis

fileID_t_half = fopen(strcat(strtok(fullfile(results_dir,filename_new),'.'),'_t_half_value_from_fit.txt'),'wt');
fprintf(fileID_t_half, '%f \n', t_half);
fclose(fileID_t_half);


fileID_t_half_2 = fopen(strcat(strtok(fullfile(results_dir,filename_new),'.'),'_t_half_value_no_fit.txt'),'wt');
